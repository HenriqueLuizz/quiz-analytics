-- Views para análise de dados do quiz (versão corrigida)
...
SELECT
  COALESCE(q.assunto, 'Não definido') as assunto,
...
FROM dim_question q
LEFT JOIN dim_alternative a ON q.question_id = a.question_id
LEFT JOIN fact_response r ON a.alternative_id = r.alternative_id
GROUP BY q.assunto
...
SELECT
  COALESCE(q.dificuldade, 'Não definido') as dificuldade,
  COUNT(DISTINCT q.question_id) AS total_questions,
  COUNT(r.response_id) as total_responses,
  ROUND(100.0 * COUNT(DISTINCT q.question_id) / (SELECT COUNT(*) FROM dim_question), 2) AS question_pct
FROM dim_question q
